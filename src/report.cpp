...
void report_status_message(u8 status_code) 
{
...
void report_alarm_message(i8 alarm_code)
{
...
void report_feedback_message(u8 message_code)
{
  printPgmString("[");
  switch(message_code) {
...
void report_grbl_settings(const settings_t& settings) {
...
void report_ngc_parameters()
{
  ...
  printPgmString("[G92:"); // Print G92,G92.1 which are not persistent in memory // Выведите G92,G92.1, которые не являются постоянными в памяти
  for (i=0; i<N_AXIS; i++) {
    printFloat_CoordValue(gc_state.coord_offset[i]);
    if (i < (N_AXIS-1)) { printPgmString(","); }
    else { printPgmString("]\r\n"); }
  } 
  printPgmString("[TLO:"); // Print tool length offset value // Значение смещения длины печатающего инструмента
...
void report_realtime_status(settings_t settings)
{
