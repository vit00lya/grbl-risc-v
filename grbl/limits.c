...
// циклов самонаведения и не будет реагировать должным образом. По запросу пользователя или при необходимости может быть
// специальная распиновка для e-stop, но обычно рекомендуется просто напрямую подключать
// ваш переключатель e-stop к контакту сброса Arduino, поскольку это наиболее правильный способ сделать это.
...
// ДОПОЛНИТЕЛЬНО: Программная процедура отключения предельного pin-кода.
  // При смене предельного pin-кода включите сторожевой таймер, чтобы создать небольшую задержку.
  ISR(LIMIT_INT_vect) { if (!(WDTCSR & (1<<WDIE))) { WDTCSR |= (1<<WDIE); } }
  ISR(WDT_vect) // Watchdog timer ISR // Контрольный таймер ISR
  {
    WDTCSR &= ~(1<<WDIE); // Disable watchdog timer.  // Отключить сторожевой таймер.
...
void limits_go_home(uint8_t cycle_mask) 
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {  
    ...
    if (bit_istrue(cycle_mask,bit(idx))) {
      // Установите цель на основе параметра max_travel. Убедитесь, что переключатели самонаведения включены в режим поиска по шкале.
      // ПРИМЕЧАНИЕ: значение параметра settings.max_travel[] сохраняется как отрицательное.
  ...
  // Установите режим поиска с заходом на посадку со скоростью поиска, чтобы быстро включить указанные концевые выключатели cycle_mask.
  ...
  do {
    ...
    // Инициализируйте и объявите переменные, необходимые для процедуры самонаведения.
    // Initialize and declare variables needed for homing routine.
    axislock = 0;
    n_active_axis = 0;
    for (idx=0; idx<N_AXIS; idx++) {
      // Установите целевое местоположение для активных осей и настройте вычисление скорости наведения.
      ...
      if (bit_istrue(cycle_mask,bit(idx))) {
        n_active_axis++;
          ...
          sys.position[idx] = 0;
        #endif
	  // Задайте целевое направление на основе маски цикла и состояния приближения к циклу самонаведения.
        // ПРИМЕЧАНИЕ: Это приводит к компиляции меньшего объема, чем любая другая реализация, которую мы пробовали.
	...
	// Примените блокировку оси к контактам ступенчатого порта, активным в этом цикле.
        // Apply axislock to the step port pins active in this cycle.
        axislock |= step_pin[idx];
      }

    }
    homing_rate *= sqrt(n_active_axis); // [sqrt(N_AXIS)] Отрегулируйте так, чтобы все отдельные оси перемещались со скоростью наведения. // [sqrt(N_AXIS)] Adjust so individual axes all move at homing rate.
    ...
    // Выполнить цикл самонаведения. Буфер планировщика должен быть пуст, что требуется для запуска цикла самонаведения.
    // Perform homing cycle. Planner buffer should be empty, as required to initiate the homing cycle.
    #ifdef USE_LINE_NUMBERS
      plan_buffer_line(target, homing_rate, false, HOMING_CYCLE_LINE_NUMBER); // Bypass mc_line(). Directly plan homing motion. // Обход функции mc_line(). Непосредственно планируйте движение самонаведения.
    #else
    ...
    do {
      ...
      st_prep_buffer(); // Check and prep segment buffer. NOTE: Should take no longer than 200us. // Проверьте и подготовьте сегментный буфер. ПРИМЕЧАНИЕ: Это займет не более 200 минут.

      // Exit routines: No time to run protocol_execute_realtime() in this loop. // Процедуры выхода: нет времени запускать protocol_execute_realtime() в этом цикле.
    ...
    st_reset(); // Immediately force kill steppers and reset step segment buffer. // Немедленно принудительно отключите степперы и сбросьте буфер сегмента шага.
    plan_reset(); // Reset planner buffer to zero planner current position and to clear previous motions. // Сброс буфера планировщика до нулевого текущего положения планировщика и очистка предыдущих перемещений.

    delay_ms(settings.homing_debounce_delay); // Delay to allow transient dynamics to dissipate. // Задержка, позволяющая ослабить динамику переходных процессов.

    // Reverse direction and reset homing rate for locate cycle(s). // Изменить направление и сбросить скорость наведения для цикла (ов) определения местоположения.
    approach = !approach;
...
// настройте маневр отключения от установленных концевых выключателей осей. Это обеспечивает
  // некоторый начальный зазор между переключателями, а также должно помочь предотвратить их ложное срабатывание
  // при включении жестких ограничений или при наличии общего ограничительного контакта более чем на одной оси.
  int32_t set_axis_position;
  // Установите положения концевых выключателей на станке. Не обновляйте оси, на которых нет хомутов.
  ...
  plan_sync_position();
  // Синхронизируйте положение планировщика с положением домашней машины.
  // Sync planner position to homed machine position.
    
  // sys.state = STATE_HOMING; // Ensure system state set as homing before returning.  // Перед возвратом убедитесь, что состояние системы настроено как самонаведение.
}
...
void limits_soft_check(float *target)
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    // Если включена функция принудительного определения начала координат, при проверке мягких ограничений необходимо учитывать направленность.
      // ПРИМЕЧАНИЕ: значение max_travel сохраняется как отрицательное
      if (bit_istrue(settings.homing_dir_mask,bit(idx))) {
    ...
    if (sys.soft_limit) {
      ...
      // объема рабочей области, поэтому просто выполните контролируемую остановку, чтобы не потерять позицию. По завершении
      // перейдите в режим тревоги.
      ...
      bit_true_atomic(sys_rt_exec_alarm, (EXEC_ALARM_SOFT_LIMIT|EXEC_CRITICAL_EVENT)); // Indicate soft limit critical event // Указывает на критическое событие с мягким ограничением
      protocol_execute_realtime(); // Execute to enter critical event loop and system abort // Выполнить для входа в цикл обработки критических событий и прерывания работы системы
      return;
    }
  }
}
